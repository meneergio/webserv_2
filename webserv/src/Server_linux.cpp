...
void Server::handleWrite(int fd) {
    ...
    if (client.send_buffer.empty()) {
        if (client.keep_alive) {
            client.request.reset();
            client.parser.reset();
            // Check pipeline buffer
            client.parser.parse(client.request, "");
            if (client.request.isComplete()) processRequest(client);
            else modifyEvent(fd, EPOLLIN);
        } else removeClient(fd);
        return;
    }
